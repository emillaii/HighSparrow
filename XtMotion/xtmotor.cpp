...
bool XtMotor::WaitArrivedTargetPos(double target_position, int timeout)
{
    ...
    qCritical("wait target_position:%f time out, current_position:%f", target_position,GetFeedbackPos());
    qCritical() << "Axis: " << name;
...
bool XtMotor::MoveToPosSync(double pos, int thread,int time_out)
{
    ...
    while ( fabs(currPos - targetPos) >= POS_ERROR)
    {
        ...
        count-=10;
        if (count < 1) {
            qCritical("Motion Timeout.target pos:%f current pos:%f",pos,currPos);
            qCritical() << "Axis: " << name;
